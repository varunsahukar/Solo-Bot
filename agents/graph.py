...
def _build_state(task: str, query: str = '', doc_id: str = '', metadata: dict = None) -> AgentState:
    return AgentState(query=query, task_type=task, doc_id=doc_id, retrieved_chunks=[], answer='', metadata=metadata or {})
...
async def run_video(doc_id: str, transcript: str = '') -> dict:
    metadata = {'transcript': transcript} if transcript else {}
    s = await app.ainvoke(_build_state('video', '', doc_id, metadata=metadata)); return {'answer': s['answer']}
